...
export async function POST(
  ...
  if (action === "create") {
    ...
    if (!value || typeof value !== "string" || value.length < 4) {
      return NextResponse.json({ error: "Code value must be at least 4 characters" }, { status: 400 });
    }

    const codes = await getUserTransferCodes(supabase, user.id);
    const maxOrder = codes.length > 0 ? Math.max(...codes.map((c) => c.sort_order)) : 0;

    const { data, error } = await createUserTransferCode(supabase, {
      user_id: user.id,
      code_type: type,
      code_hash: hashCode(value),
    ...
    return NextResponse.json({
      success: true,
      code: { id: data?.id, code_type: type, sort_order: data?.sort_order },
    });
  }

  if (action === "update") {
    const { id, value, sort_order } = body;
    if (!id) return NextResponse.json({ error: "id required" }, { status: 400 });

    const updates: { code_hash?: string; sort_order?: number } = {};
    if (value !== undefined) {
      if (typeof value !== "string" || value.length < 4) {
        return NextResponse.json({ error: "Code value must be at least 4 characters" }, { status: 400 });
      }
      updates.code_hash = hashCode(value);
    }
